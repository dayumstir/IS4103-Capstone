...
export const searchNotifications = async (searchQuery: string) => {
    ...
    const notifications =
        await notificationRepository.listAllNotificationsWithSearch(
            searchQuery
        );
...
export const getMerchantNotifications = async (
    ...
    logger.info(`Searching for notifications with query: ${searchQuery}`);
    const notifications = await notificationRepository.getMerchantNotifications(
        merchantId,
        searchQuery
    );
    return notifications;
...
export const updateNotification = async (notificationData: INotification) => {
    ...
    const notification =
        await notificationRepository.updateNotification(notificationData);
    return notification;
};
