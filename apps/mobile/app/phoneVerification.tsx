...
import { useVerifyPhoneNumberOTPMutation } from "../redux/services/customerAuthService";
...
export type PhoneVerificationFormValues = z.infer<
  typeof phoneVerificationSchema
>;

export default function PhoneVerificationScreen() {
  const dispatch = useDispatch();
  const [verifyPhoneNumberOTPMutation, { isLoading, error }] =
  ...
  const onSubmit = async (data: PhoneVerificationFormValues) => {
    try {
      const result = await verifyPhoneNumberOTPMutation({
        otp: data.otp,
      }).unwrap();
      ...
      <View className="flex w-full flex-col gap-8 rounded-lg bg-white p-8 shadow-md">
        ...
        <View className="flex gap-4">
          <Text className="text-center leading-normal text-gray-800">
            We've sent an SMS to your registered contact number. Please type in
            the OTP in the SMS to verify your contact number.
          </Text>
        </View>
        ...
        {/* Error Handling */}
        {error && (
          <Text className="mt-1 text-red-500">Invalid OTP provided</Text>
        )}
      </View>
    </View>
  );
